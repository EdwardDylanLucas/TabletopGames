...
class SushiBasicTreeNode {
    // Root node of tree
    SushiBasicTreeNode root;
    // Parent of this node
    SushiBasicTreeNode parent;
    // Children of this node
    Map<AbstractAction, SushiBasicTreeNode> children = new HashMap<>();
    // Depth of this node
    final int depth;
    ...
    private Random rnd;
    // private RandomPlayer rolloutPlayer = new RandomPlayer();
//    private ActionValueSushiGoPlayer rolloutPlayer = new ActionValueSushiGoPlayer();
    private ActionValueSushiGoPlayer rolloutPlayer = new ActionValueSushiGoPlayer();
    ...
    protected SushiBasicTreeNode(BasicMCTSPlayer player, SushiBasicTreeNode parent, AbstractGameState state, Random rnd) {
        this.player = player;
        this.fmCallsCount = 0;
        this.parent = parent;
    ...
    void mctsSearch() {
        ...
        while (!stop) {
            ...
            SushiBasicTreeNode selected = treePolicy();
     ...
     */
    private SushiBasicTreeNode treePolicy() {

        SushiBasicTreeNode cur = this;
     ...
     */
    private SushiBasicTreeNode expand() {
        ...
        SushiBasicTreeNode tn = new SushiBasicTreeNode(player, this, nextState, rnd);
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : children.keySet()) {
            SushiBasicTreeNode child = children.get(action);
            if (child == null)
    ...
    private double rollOut() {
        ...
        if (player.getParameters().rolloutLength > 0) {
            while (!finishRollout(rolloutState, rolloutDepth)) {
                ...
                AbstractAction next = rolloutPlayer.getAction(rolloutState, rolloutPlayer.getForwardModel().computeAvailableActions(rolloutState, rolloutPlayer.parameters.actionSpace));
    ...
    AbstractAction bestAction() {
        ...
        for (AbstractAction action : children.keySet()) {
            if (children.get(action) != null) {
                SushiBasicTreeNode node = children.get(action);
